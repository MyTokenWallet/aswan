...
class TestSampleFunction(unittest.TestCase):
    ...
    def test_is_abnormal(self):
        ...
        for req_body, result in testcases:
            self.assertEqual(is_abnormal(req_body), result)
    ...
    def test_user_login_count(self):
        ...
        for req_body, result in testcases:
            self.assertEqual(user_login_count(req_body), result)
...
class TestBaseFunction(unittest.TestCase):
    ...
    def test_is_and_is_not(self):
        ...
        for req_body, result in testcases:
            self.assertEqual(BuiltInFuncs.run(req_body, func_code, 'is'),
                              False if result is None else result)
            self.assertEqual(BuiltInFuncs.run(req_body, func_code, 'is_not'),
    ...
    def test_args_not_enough(self):
        ...
        self.assertEqual(BuiltInFuncs.run(req_body, func_code, 'is', 1), False)

    def test_args_method(self):
        func_code = 'is_abnormal'
        self.assertEqual(BuiltInFuncs.get_required_args(func_code),['user_id'])

    def test_repr(self):
        func_code = 'is_abnormal'
        buildin_func_obj = BuiltInFuncs.name_callable.get(func_code)
        self.assertEqual(repr(buildin_func_obj), buildin_func_obj.desc)
