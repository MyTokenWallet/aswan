...
class BaseForm(FormBaseMixin, forms.Form):
    ...
    form_inputs = [Submit('submit', 'Save'),
                   Reset('reset', 'Reset')]
...
class BaseFilterForm(FormBaseMixin, forms.Form):
    ...
    def get_layout(self, helper):
        ...
        action = FormActions(Submit('submit', 'Submit'), css_class="form-group")
        items.append(action)
        layout = Layout(*items)
        return layout
...
class BaseTimeFilterForm(forms.Form):
    time_start = forms.CharField(label="StartTime", required=False)
    time_end = forms.CharField(label="EndTime", required=False)
